-- SQLite-compatible version of the database schema and data

-- Drop existing views and tables one by one to ensure a clean start
...
-- Create all tables with SQLite-specific syntax
...
CREATE TABLE orders (
    ...
    items_json TEXT, -- Changed from JSONB to TEXT for SQLite
    total REAL,
    payment_method TEXT,
    delivery_date TEXT,
...
-- Insert orders
INSERT INTO orders (id, customer_id, items_json, total, payment_method, delivery_date, status, cancellation_timestamp, order_date, actual_delivery_date, cancellation_refund) VALUES
